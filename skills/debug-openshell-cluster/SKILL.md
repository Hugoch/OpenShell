...
must receive both sandbox configuration and provider environment in the
bootstrap. It acknowledges application only after boundary enforcement, the
canonical process, and SSH are ready. If the supervisor Pod is running but the
sandbox remains Starting, inspect gateway and supervisor logs for bootstrap
build failures, missing components, revision mismatches, apply failures, or the
two-minute bootstrap timeout. Do not add gateway credentials to the workload
Pod; configuration delivery terminates in the separate supervisor.
