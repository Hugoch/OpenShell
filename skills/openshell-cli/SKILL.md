...
- `--cpu`, `--memory`: Set portable per-sandbox quantities. Docker/Podman apply them as limits. Kubernetes applies them as limits and uses them as matching requests unless Kubernetes driver config sets an explicit request.
