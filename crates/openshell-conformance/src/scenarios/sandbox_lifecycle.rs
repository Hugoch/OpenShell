...
fn run_sandbox_lifecycle(runner: &mut OpenShellRunner) -> ScenarioFuture<'_> {
    Box::pin(async move {
        SANDBOX_LIFECYCLE_STATE_TRANSITIONS_SCENARIO
            .run(runner)
            .await?;
        SANDBOX_LIFECYCLE_RELAY_READINESS_SCENARIO.run(runner).await
    })
}
...
fn run_relay_readiness(runner: &mut OpenShellRunner) -> ScenarioFuture<'_> {
    Box::pin(async move {
        match std::env::var_os("OPENSHELL_RELAY_RECONNECT_HELPER")
            .filter(|helper| !helper.is_empty())
        {
            Some(helper) => relay_readiness_with_forced_reconnect(runner, &helper).await,
            None => relay_readiness_survives_reconnects(runner).await,
        }
    })
}

async fn relay_readiness_with_forced_reconnect(
    ...
    create_running_sandbox(
        ...
        &sandbox_name,
        "exec sleep infinity",
        "relay-readiness/forced-reconnect",
    )
    .await?;
    ...
    let relay_command = runner
        .step("relay-readiness/forced-reconnect/exec")
        .description(format!(
    ...
    let reconnect = async {
        // Allow the gateway to queue RelayOpen while the supervisor is paused,
        // then sever only the control connection. The same process and
        // workload remain alive and the supervisor reconnects after unpause.
    ...
    run_lifecycle_command(
        ...
        "delete",
        &sandbox_name,
        "relay-readiness/forced-reconnect/delete",
    )
    .await?;
    wait_for_absence(
        runner,
        &sandbox_name,
        "relay-readiness/forced-reconnect/deleted",
    )
    .await?;
