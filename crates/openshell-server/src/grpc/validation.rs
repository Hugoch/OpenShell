...
fn validate_resource_requirement_fields(spec: &SandboxSpec) -> Result<(), Status> {
    ...
    {
        if let Some(quantity) = cpu.quantity.as_deref() {
            validate_cpu_quantity(quantity, "spec.resource_requirements.cpu.quantity")?;
        } else {
            return Err(Status::invalid_argument(
                "spec.resource_requirements.cpu must set quantity",
            ));
        }
    }
    ...
    {
        if let Some(quantity) = memory.quantity.as_deref() {
            validate_memory_quantity(quantity, "spec.resource_requirements.memory.quantity")?;
        } else {
            return Err(Status::invalid_argument(
                "spec.resource_requirements.memory must set quantity",
            ));
        }
    }
    ...
    for section_name in ["limits", "requests"] {
        ...
        for resource_name in ["cpu", "memory"] {
            if section.fields.contains_key(resource_name) {
                return Err(Status::invalid_argument(format!(
                    "template.resources.{section_name}.{resource_name} is no longer supported; use spec.resource_requirements.{resource_name}.quantity"
                )));
            }
        }
    }
...
mod tests {
    ...
    fn validate_sandbox_spec_rejects_invalid_cpu_requirements() {
        ...
        assert!(
            err.message()
                .contains("spec.resource_requirements.cpu.quantity")
        );
    }

    #[test]
    fn validate_sandbox_spec_accepts_cpu_and_memory_quantities() {
        let spec = SandboxSpec {
            resource_requirements: Some(openshell_core::proto::ResourceRequirements {
                gpu: None,
                cpu: Some(openshell_core::proto::CpuResourceRequirements {
                    quantity: Some("250m".to_string()),
                }),
                memory: Some(openshell_core::proto::MemoryResourceRequirements {
    ...
    #[test]
    fn validate_sandbox_spec_rejects_invalid_memory_quantity() {
        let spec = SandboxSpec {
            resource_requirements: Some(openshell_core::proto::ResourceRequirements {
                gpu: None,
                cpu: None,
        ...
        assert!(
            err.message()
                .contains("spec.resource_requirements.memory.quantity")
        );
    }
