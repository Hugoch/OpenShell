...
fn parse_podman_cpu_limit(
    ...
    let Some(quantity) = cpu.quantity.as_deref() else {
        return Ok(None);
    };
    parse_cpu_to_microseconds(quantity)
        .map(Some)
        .ok_or_else(|| {
...
fn parse_podman_memory_limit(
    ...
    let Some(quantity) = memory.quantity.as_deref() else {
        return Ok(None);
    };
    parse_memory_to_bytes(quantity).map(Some).ok_or_else(|| {
        ComputeDriverError::Precondition(format!(
