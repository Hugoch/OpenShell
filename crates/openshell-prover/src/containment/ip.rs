...
mod tests {
    ...
    fn check(host: &str, maximum: &str, candidate: &str) -> CheckResult {
        check_within_maximum(&policy(host, maximum), &policy(host, candidate), options())
    }
    ...
    fn defaults_and_aliases_use_runtime_modes() {
        ...
        let result = check_within_maximum(
            &policy("*.example.com", ""),
    ...
    fn malformed_blocked_and_order_dependent_inputs_are_unsupported() {
        ...
        ambiguous
            ...
            .push(second);
        assert!(matches!(
            check_within_maximum(&ambiguous, &ambiguous, options()),
            CheckResult::Unsupported(_)
        ));
    }
    ...
    fn excessive_ip_ranges_fail_before_constructing_the_model() {
        ...
        assert!(
            matches!(check_within_maximum(&huge, &huge, options()), CheckResult::Inconclusive(ref reason) if reason.reason_code() == super::super::ReasonCode::ResourceLimit)
        );
    }
}
