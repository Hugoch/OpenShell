...
pub fn build_cpu_resource_requirements(
    ...
    Ok(Some(CpuResourceRequirements {
        quantity: Some(validate_cpu_quantity(cpu)?),
    }))
}
...
pub fn build_memory_resource_requirements(
    ...
    Ok(Some(MemoryResourceRequirements {
        quantity: Some(validate_memory_quantity(memory)?),
    }))
}
...
pub async fn sandbox_template_create(
    ...
    let resources = if cpu.is_some() || memory.is_some() || gpu_requirements.is_some() {
        Some(ResourceRequirements {
            cpu: cpu.map(validate_cpu_quantity).transpose()?.map(|quantity| {
            ...
            memory: memory
                ...
                .transpose()?
                .map(|quantity| MemoryResourceRequirements {
                    quantity: Some(quantity),
                }),
            ...
            if let Some(resources) = &workload.resources {
                ...
                if let Some(cpu) = &resources.cpu {
                    resources_json.insert(
                        "cpu".to_string(),
                        serde_json::json!({ "quantity": cpu.quantity }),
                    );
                }
                if let Some(memory) = &resources.memory {
                    resources_json.insert(
                        "memory".to_string(),
                        serde_json::json!({ "quantity": memory.quantity }),
                    );
                }
...
mod tests {
    ...
    fn build_cpu_resource_requirements_sets_typed_limit() {
        ...
        assert_eq!(cpu.quantity.as_deref(), Some("500m"));
    }
    ...
    fn build_memory_resource_requirements_sets_typed_limit() {
        ...
        assert_eq!(memory.quantity.as_deref(), Some("2Gi"));
    }
