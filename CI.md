...
Kubernetes conformance is temporarily quarantined for [#3421](https://github.com/NVIDIA/OpenShell/issues/3421). Required Kubernetes lanes run the `smoke` scenario, while `Kubernetes lifecycle advisory (#3421)` continues to run `sandbox-lifecycle` without contributing to `Core E2E result`. Remove the advisory lane and the `smoke` selections when #3421 is fixed.
