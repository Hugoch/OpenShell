...
memory quantities as runtime limits. Kubernetes applies each quantity as a
limit and mirrors it into the matching request unless driver configuration
provides that request. VM and MXC reject unsupported CPU and memory requirements instead of
...
and forwards only that block to the driver. The gateway retains the former
workload-resource wire field solely at the storage boundary: persisted
templates decode through that legacy shape and are normalized to current
`ResourceRequirements` before Get, List, or sandbox creation. New API writes
cannot populate the legacy field.
...
single GPU. CPU and memory requirements each carry one optional portable `quantity`
(e.g. `"500m"`, `"4Gi"`). Docker and Podman apply quantities as limits. Kubernetes
applies them as limits and uses driver configuration for an independently tuned or
request-only value, falling back from the quantity when no driver-config request is set.
The VM and MXC drivers reject typed CPU/memory
