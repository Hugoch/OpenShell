...
zero) are acknowledged through the policy status API. The configuration stream
separately acknowledges complete snapshot application for every component.
Global policies and local-file development policies do not use the sandbox
revision API. When explicit local Rego and data files are provisioned into the
supervisor, streamed settings, middleware, and provider state still apply, but
gateway policy revisions never replace the local OPA engine. Workload image
files and environment variables do not configure the separately isolated
supervisor.
