...
<Note>
**Air-gapped clusters:** mirror the manifest above and the `registry.k8s.io/agent-sandbox/agent-sandbox-controller` image referenced inside it to your internal registry, then point the manifest's image reference at your mirror before applying. You will also need to mirror the OpenShell gateway, supervisor, and sandbox images — see the chart's `gateway.image`, `supervisor.image`, and `sandbox.image` values.
</Note>
...
| Value | Purpose |
|---|---|
| `gateway.image.repository` / `gateway.image.tag` / `gateway.image.digest` | Gateway container image. Defaults to `ghcr.io/nvidia/openshell/gateway:latest`. |
...
| `supervisor.image.repository` / `supervisor.image.tag` / `supervisor.image.digest` | Supervisor image used to provide the `openshell-sandbox` binary. |
