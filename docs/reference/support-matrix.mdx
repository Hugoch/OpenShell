...
| Helm value | Purpose |
|---|---|
| `gateway.image.repository` / `gateway.image.tag` / `gateway.image.digest` | Override the gateway image reference. |
| `supervisor.image.repository` / `supervisor.image.tag` / `supervisor.image.digest` | Override the supervisor image reference. |
| `sandbox.image.repository` / `sandbox.image.tag` / `sandbox.image.digest` | Override the default sandbox image. |
