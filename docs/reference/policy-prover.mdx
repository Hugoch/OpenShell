...
The `maximum-boundary-v2` containment model covers filesystem paths, process
...
`exceeds_max` with the field and both values. Other identity changes return
...
`exceeds_max`. Keeping the same mode or strengthening it to `hard_requirement`
passes this part of the check. This compares the requested requirement, not
whether a target kernel successfully installed Landlock restrictions.
...
Consumers must explicitly accept model `maximum-boundary-v2` and its additional
`process` and `landlock` domains. Do not treat a v1 result as covering these fields.
