...
ProtoJSON treats YAML `null` as an unset field. Do not use `null` for matchers
or selectors. For example, `tool: null` removes the tool selector instead of
matching a null value. `version: null` becomes the default value `0` and fails
policy validation.
...
- Policy YAML follows protobuf presence semantics. Omitted, `null`, and empty `mcp.versions` values all select the pinned `["2025-11-25"]` default because protobuf repeated fields do not preserve field presence.
