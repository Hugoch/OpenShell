...
| `network_budgets` | map | No | Dynamic | Caps how much the sandbox uses allowed egress: requests, connections, and bytes per period. |
| `usage_monitoring` | object | No | Dynamic | Sets the novelty learning period and the drift thresholds for egress usage findings. |
...
A map of up to 32 named budgets on allowed egress. The map key is the budget name. A budget keeps its balance across policy reloads while its name stays the same. Budgets act when a connection or an L7 request starts. They do not stop a transfer in progress. Refer to [Egress Usage](/observability/egress-usage) for the counted bytes, the findings, and the responses.
...
| Field | Type | Required | Description |
|---|---|---|---|
| `policies` | list of strings | No | Keys of `network_policies` entries. The budget selects traffic that any of these policies allowed. Empty selects traffic from every policy. Each key must exist in `network_policies`. |
| `hosts` | list of strings | No | Exact host or DNS glob patterns on the destination host, at most 32. Empty selects every host. |
| `requests_per_minute` | integer | No | L7 requests per minute. Applies only to endpoints where OpenShell parses HTTP. |
| `write_requests_per_minute` | integer | No | L7 write requests per minute: HTTP methods other than `GET`, `HEAD`, and `OPTIONS`, and GraphQL mutations. A write also takes a token from `requests_per_minute`. |
| `connections_per_minute` | integer | No | New connections per minute. |
| `bytes_out_per_hour` | integer | No | Bytes that the sandbox sends per hour. |
| `bytes_in_per_hour` | integer | No | Bytes that the sandbox receives per hour. |
| `on_exceed` | string | No | `alert` records a finding and admits the traffic. `deny` records a finding and refuses the traffic. Defaults to `alert`. |

A budget needs at least one counter. An absent counter has no ceiling. Zero is rejected: to block an endpoint, remove it from `network_policies`.

Each counter is a token bucket. The capacity is the amount for one period, and the bucket refills at a constant rate over that period. A budget is therefore a sustained rate with a burst of one period, not a fixed ceiling per calendar minute or hour. Byte counters charge bytes as they move, so one large transfer can overrun a byte budget. The next connection or request then waits until the refill pays back the debt.

## Usage Monitoring

**Category:** Dynamic

Settings for the novelty and drift findings that OpenShell produces from egress usage. All fields are optional.
...
|---|---|---|---|
| `novelty.learning_period_seconds` | integer | `600` | Time after a network policy starts, or after its endpoints change, during which a first use produces no finding. |
| `drift.enabled` | boolean | `true` | Whether the gateway computes drift findings. |
| `drift.ratio` | integer | `10` | A counter drifts when one window is more than this many times its baseline. Must be at least `2`. The baseline is the sandbox baseline, or the cohort baseline before the sandbox baseline is warm. |
| `drift.min_requests` | integer | `100` | Smallest request baseline that drift compares against. |
| `drift.min_bytes` | integer | `104857600` | Smallest byte baseline that drift compares against. |
