...
Add a client WebSocket text-message deny. The path matches the stored upgrade
request path, not message content:
...
Use `--dry-run` to review the merged document:
...
Use `policy set` for middleware changes, advanced protocol shapes, or complete
replacement. Start from the current base and preserve sections you do not intend
to change. Follow [Inspect and Replace a
Policy](/sandboxes/policies#inspect-and-replace-a-policy) to prepare an editable
...
Edit `demo-base.json`, then submit the complete policy:

```shell
openshell policy set demo --policy demo-base.json --wait
```

Refer to [Customize Sandbox Policies](/sandboxes/policies#inspect-and-replace-a-policy)
