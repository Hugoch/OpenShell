...
| `supervisor_image` | `supervisor.image.repository` / `supervisor.image.tag` / `supervisor.image.digest` | Override the image that provides `openshell-supervisor`. An empty tag defaults to the chart appVersion; a digest takes precedence over the tag. |
