...
The agent workload uses `network=none`. Its trusted supervisor companion uses Podman's host network for its gateway session and policy-approved upstream connections. DNS queries from the workload use the sandbox's loopback resolver and remain subject to policy, including requests to `host.openshell.internal`.
