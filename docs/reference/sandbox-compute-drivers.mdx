...
- `cpu.quantity` and `memory.quantity` are optional portable quantities.
...
|---|---|
| Docker | Converts typed quantities to native container CPU and memory limits. |
| Podman | Converts typed quantities to native container CPU and memory limits. |
| Kubernetes | Applies typed quantities as limits. A quantity also becomes the request unless a Kubernetes driver-config request is set; driver config can also specify request-only resources. |
...
the corresponding typed quantity. Kubernetes driver configuration may set
explicit CPU and memory requests. These configured requests replace the request
that Kubernetes would otherwise mirror from the portable quantity. The
portable quantity remains the limit.

## Resource Capability Reporting

API clients can read each configured driver's static portable resource support
from the protected `GetGatewayInfo` response. The capability response describes
which portable resource inputs a driver accepts. It does not report live
resource inventory, availability, or scheduling capacity, so a supported
request can still fail when the selected runtime cannot provide the resource.

CPU and memory capabilities report whether the driver accepts a portable
quantity. GPU capabilities report whether the driver accepts a default GPU
request and an explicit GPU count. An omitted `resource_capabilities` message,
or an omitted CPU, memory, or GPU capability, means the driver did not report
that capability. A reported `false` value means that input is unsupported.
