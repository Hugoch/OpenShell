...
Supervisor middleware adds ordered processing stages to allowed HTTP and
WebSocket egress. Request stages run after network and L7 policy admit traffic
and before OpenShell injects provider credentials. Response stages run on the
final upstream HTTP response before return. A stage can allow, deny, transform
supported payloads, mutate permitted request headers, and report audit-safe
findings according to the operation it advertises.

Middleware selection is independent of the network policy rule that admitted the
request. OpenShell first matches attachments by destination host, then selects
only operations advertised by each implementation. A host match therefore does
not mean that every request, response, or WebSocket message is inspected. Refer
to the [conceptual inspected HTTP flow](/sandboxes/network-policy-recipes#understand-inspected-http-flow)
for the relationship between policy, middleware, credentials, and upstream
responses.
...
Registration is loaded at gateway startup. Restart the gateway after adding,
removing, or changing a registration. A sandbox policy can add, remove, or
reconfigure its use of an already registered service without restarting the
gateway. Built-in middleware requires no registration. See
[Gateway Configuration](/reference/gateway-config#supervisor-middleware-services)
for the complete gateway TOML context.
...
- Use `policy set` to add, remove, or reconfigure a sandbox's use of built-in or already registered middleware. These policy changes can activate while the sandbox runs.
