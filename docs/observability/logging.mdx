...
| `NET:` | Network Activity | 4001 | TCP proxy CONNECT tunnels, connection close totals, bypass detection, DNS failures |
...
| `FINDING:` | Detection Finding | 2004 | Security findings (nonce replay, proxy bypass, unsafe policy) and egress usage findings |
...
A closed connection with its traffic totals:

```text
OCSF NET:CLOSE [INFO] ALLOWED api.github.com:443 [policy:github_api engine:opa] [bytes_out:512 bytes_in:4096 duration_ms:1500]
```

A request refused because an egress budget has no balance left:

```text
OCSF FINDING:BLOCKED [MED] "api.github.com:443 budget 'github-requests' has no requests_per_minute left" [type:egress.budget_exceeded]
```
...
- Read [egress usage](/observability/egress-usage) and budget findings.
