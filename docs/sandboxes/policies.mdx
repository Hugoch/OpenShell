...
A policy has static sections `filesystem_policy`, `landlock`, and `process` that are locked at sandbox creation, and dynamic `network_policies`, `network_middlewares`, `network_budgets`, and `usage_monitoring` sections that are hot-reloadable on a running sandbox.
...
| `network_budgets` | Dynamic | Declares named budgets on allowed egress: requests and connections per minute, and bytes in each direction per hour. A budget with `on_exceed: deny` refuses traffic without balance with HTTP 429. Refer to [Egress Budgets](#egress-budgets). |
| `usage_monitoring` | Dynamic | Sets the learning period for first-use findings and the thresholds for drift findings. Refer to [Egress Usage](/observability/egress-usage). |
...
Network policy decides where the sandbox can connect. Budgets limit how much it uses an allowed destination. A budget selects traffic by the `network_policies` keys that allowed it and, optionally, by destination host:
...
network_budgets:
  api-requests:
    ...
    on_exceed: deny
  downloads:
    bytes_in_per_hour: 10737418240
  package-writes:
    policies: [package_proxy]
    write_requests_per_minute: 5
    on_exceed: deny
```

A `write_requests_per_minute` counter limits only requests that can change remote state, such as `PUT`, `POST`, and WebDAV `MKCOL`. Use it on an endpoint that the agent mostly reads, for example a package proxy. A read-only L7 rule is stronger when the agent must never write.

With `on_exceed: deny`, a request without balance gets `429 Too Many Requests` with a `Retry-After` header and an `error: budget_exceeded` body. The default `on_exceed: alert` records a finding and admits the traffic, so you can add a budget to a running sandbox, observe it, and then change it to `deny`. A budget keeps its balance across reloads while its name stays the same.

Budgets act when a connection or request starts and never stop a transfer in progress. Refer to the [Policy Schema Reference](/reference/policy-schema#network-budgets) for every field and to [Egress Usage](/observability/egress-usage) to read usage, budget denials, and findings.
...
Use `openshell policy update` when you want to merge network policy changes into the current live policy instead of replacing the whole YAML document. This command only updates the dynamic `network_policies` section. Change `network_middlewares`, `network_budgets`, or `usage_monitoring` with `openshell policy set`.
