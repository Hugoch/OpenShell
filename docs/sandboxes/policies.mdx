...
A sandbox policy expresses access rules and runtime settings in declarative
YAML. Filesystem settings define where programs can read and write, and process
...
The following outline shows how these controls are organized, with individual
settings omitted:
...
You can supply a policy when creating a sandbox, update its network rules, or
