...
Set portable CPU and memory quantities with `--cpu` and `--memory`:
...
and Podman apply them as runtime limits. Kubernetes uses each quantity as its
limit and, unless its driver configuration provides a request, as its request.
The VM and MXC drivers reject
...
API and SDK callers set `cpu.quantity` and `memory.quantity`. Workloads that
need Kubernetes requests below those portable quantities can set requests in
the Kubernetes driver configuration. A driver-config request takes precedence
over the quantity-to-request fallback.
...
use `SandboxSpec.resource_requirements.cpu.quantity` and
`SandboxSpec.resource_requirements.memory.quantity`. The gateway
...
and SDK callers use `cpu.quantity`, `memory.quantity`, and `gpu` in both places.
When loading a reusable template stored with the earlier `cpu`, `memory`, and
`gpu` shape, the gateway converts it to the current typed requirements before
returning or using the template.
