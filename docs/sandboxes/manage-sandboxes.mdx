...
### Inspect Egress Usage

Show how much the sandbox uses each allowed destination, with budget denials and usage findings:

```shell
openshell sandbox usage my-sandbox
```

In OpenShell Terminal, open the sandbox and press `u` for the same view. Refer to [Egress Usage](/observability/egress-usage) for the counters and findings.
