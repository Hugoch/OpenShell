...
If the main process has exited and its terminal is no longer attachable,
`connect` reports `canonical main process already finished` and returns status 1.
A pending foreground attachment can still retrieve retained output and the
process's exit status after the sandbox reaches `Completed` or `Error`.
