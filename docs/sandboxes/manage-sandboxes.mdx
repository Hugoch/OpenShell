...
If a gateway connection drops during a transfer, the CLI retries the whole
upload or download with a fresh sandbox connection. It can also wait briefly
for the sandbox to become ready again after a gateway restart. A sandbox that
is already stopped still returns an error immediately.
