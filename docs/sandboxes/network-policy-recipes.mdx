...
Each recipe illustrates a permission and how to verify it. The commands use
`my-sandbox` as a placeholder for your running sandbox's name. The sandbox must
...
status metadata above it. Keep all existing sections. Insert the recipe under
the top-level `network_policies` map, with its rule name indented two spaces.
Add that map if it is absent. If it is empty (`network_policies: {}`), replace
`{}` with the indented recipe on the following lines. Replace example hosts,
paths, and binaries with your own values. Each recipe is a rule entry, not a
