...
Use these recipes to choose what OpenShell inspects and to define both an
allowed and denied outcome. Every YAML block is a complete entry for the
`network_policies` map in a `version: 1` policy file.

## Choose a Protocol

Layer 4 is a connection check over a host, port, and executable. Layer 7 is an
application-request check after connection admission.
...
Apply each recipe before running its verification steps. The commands use a
running sandbox named `demo`, `jq`, and Mike Farah `yq` v4. The sandbox must
contain the binary named by the recipe, and its container engine must be able to
use the sandbox image. Recipes with example domains also require a service you
control at the replacement hostname and path.

Export the editable base policy. This preserves startup settings and excludes
...
Save one recipe block as `recipe.yaml`. Convert it to JSON, merge the rule under
`network_policies`, and inspect the result:

```shell
yq --output-format json '.' recipe.yaml > recipe.json
jq --slurpfile recipe recipe.json \
  '.network_policies = ((.network_policies // {}) + $recipe[0])' \
  demo-base.json > demo-policy.json
jq '.network_policies' demo-policy.json
```

The merge replaces an existing rule with the same map key. After reviewing the
result, apply the complete policy and confirm which revision is active:

```shell
openshell policy set demo --policy demo-policy.json --wait
openshell policy list demo
openshell policy get demo --full
```

Use [Incremental Policy Updates](/reference/policy-updates) instead when its
REST and WebSocket operations cover the change. A successful parse or submission
...
The destination and process check gates the connection. Protocol rules then
check the application request, and optional middleware and credential bindings
can impose additional restrictions. See the recipes below for native TCP and
WebSocket boundaries that differ from ordinary inspected HTTP requests.
...
POST with the tutorial sandbox's `/usr/bin/curl`:
...
Use audit mode only when you deliberately want to observe applicable request
violations without blocking them. Change the REST endpoint above to
`enforcement: audit`, apply the policy, and repeat the POST. The request passes
the request-policy check and continues to the upstream service. Its eventual
HTTP status is therefore an upstream result, not proof that OpenShell allowed
the operation semantically.
...
The event must identify the request-policy violation even though the request
was forwarded. OCSF policy events are INFO-level tracing records; their OCSF
severity does not change the CLI's tracing-level filter. Audit does not bypass
destination, executable, credential, parser, or middleware checks. Return the
endpoint to `enforcement: enforce` before relying on the rule as a boundary.
...
Docker and Podman support policy DNS and transparent capture. The standard
runtime initializes that substrate even when the initial policy has no TCP
endpoints, so you can add the first endpoint through a live update. An alternate
backend must advertise both the capability and a ready substrate.
...
The commands below are an adaptable template. Replace `api.example.com` and the
paths with routes on an HTTP service you control. Suppose one rule permits `GET
...
exceptions close enough to review together. After applying both rules with the
shared procedure, test the overlap explicitly:
