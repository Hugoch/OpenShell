...
### Connection and Request Inspection

The `protocol` field on an endpoint controls whether the proxy inspects individual HTTP requests inside the tunnel.

| Aspect | Detail |
|---|---|
| Default | Endpoints without a `protocol` field define no protocol-specific request rules. The explicit proxy still checks the host, port, and binary, auto-detects and terminates TLS, and checks the destination authority when it parses HTTP. Provider-credentialed endpoints reject traffic without an inspected protocol unless an operator explicitly opts in. |
...
| Aspect | Detail |
|---|---|
| Restrictive fallback | `/bin`, `/usr`, `/lib`, `/proc`, `/dev/urandom`, `/etc`, and `/var/log` are read-only. The resolved working directory, `/tmp`, and `/dev/null` are read-write. Runtime enrichment can add paths required by the selected driver. |
| What you can change | Choose paths at creation. After activation, additions may be accepted for a later launch, but removals and baseline changes are rejected; recreate the sandbox for an assured new startup boundary. |
...
|---------|---------------|-------------------|
| Omitting an inspected protocol on REST or WebSocket API endpoints | The explicit proxy still handles TLS and checks HTTP destination authority, but it cannot enforce individual methods, paths, or WebSocket text messages. | Add `protocol: rest` or `protocol: websocket` with specific `rules` to enable request-level control. |
