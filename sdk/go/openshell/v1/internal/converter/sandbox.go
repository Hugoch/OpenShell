...
func resourceRequirementsFromProto(rr *pb.ResourceRequirements) *types.ResourceRequirements {
	...
	if cpu := rr.GetCpu(); cpu != nil {
		result.CPU = &types.CPUResourceRequirements{Quantity: cpu.GetQuantity()}
	}
	if mem := rr.GetMemory(); mem != nil {
		result.Memory = &types.MemoryResourceRequirements{Quantity: mem.GetQuantity()}
	}
	return result
}
...
func resourceRequirementsToProto(rr *types.ResourceRequirements) *pb.ResourceRequirements {
	...
	if rr.CPU != nil {
		result.Cpu = &pb.CpuResourceRequirements{Quantity: StringPtr(rr.CPU.Quantity)}
	}
	if rr.Memory != nil {
		result.Memory = &pb.MemoryResourceRequirements{Quantity: StringPtr(rr.Memory.Quantity)}
	}
	return result
}
