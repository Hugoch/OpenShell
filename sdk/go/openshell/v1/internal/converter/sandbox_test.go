...
func TestSandboxFromProto(t *testing.T) {
	...
	proto := &pb.Sandbox{
		...
		Spec: &pb.SandboxSpec{
			...
			ResourceRequirements: &pb.ResourceRequirements{
				...
				Cpu:    &pb.CpuResourceRequirements{Quantity: proto.String("500m")},
				Memory: &pb.MemoryResourceRequirements{Quantity: proto.String("2Gi")},
			},
	...
	assert.Equal(t, "500m", s.Spec.ResourceRequirements.CPU.Quantity)
	require.NotNil(t, s.Spec.ResourceRequirements.Memory)
	assert.Equal(t, "2Gi", s.Spec.ResourceRequirements.Memory.Quantity)
...
func TestSandboxToProto(t *testing.T) {
	...
	s := &v1.Sandbox{
		...
		Spec: v1.SandboxSpec{
			...
			ResourceRequirements: &v1.ResourceRequirements{
				...
				CPU:    &v1.CPUResourceRequirements{Quantity: "1"},
				Memory: &v1.MemoryResourceRequirements{Quantity: "4Gi"},
			},
			Command: []string{"/opt/agent", "--serve"},
			TTY:     false,
		},
	...
	assert.Equal(t, "1", p.Spec.ResourceRequirements.Cpu.GetQuantity())
	require.NotNil(t, p.Spec.ResourceRequirements.Memory)
	assert.Equal(t, "4Gi", p.Spec.ResourceRequirements.Memory.GetQuantity())
...
func TestSandboxWorkloadTemplateRoundTrip(t *testing.T) {
		...
		Spec: v1.SandboxWorkloadTemplateSpec{
			Workload: &v1.SandboxWorkloadConfig{
				...
				Resources: &v1.ResourceRequirements{
					CPU:    &v1.CPUResourceRequirements{Quantity: "2"},
					Memory: &v1.MemoryResourceRequirements{Quantity: "8Gi"},
					GPU:    &v1.GPUResourceRequirements{Count: &gpuCount},
				},
			},
	...
	assert.Equal(t, "2", protoTemplate.Spec.Workload.Resources.Cpu.GetQuantity())
	require.NotNil(t, protoTemplate.Spec.Workload.Resources.Memory)
	assert.Equal(t, "8Gi", protoTemplate.Spec.Workload.Resources.Memory.GetQuantity())
...
func TestSandboxRoundTrip(t *testing.T) {
	...
	assert.Equal(t, original.Spec.ResourceRequirements.CPU.Quantity, back.Spec.ResourceRequirements.CPU.Quantity)
	require.NotNil(t, back.Spec.ResourceRequirements.Memory)
	assert.Equal(t, original.Spec.ResourceRequirements.Memory.Quantity, back.Spec.ResourceRequirements.Memory.Quantity)
...
func TestSandboxSpecToProto(t *testing.T) {
	...
	spec := &v1.SandboxSpec{
		...
		ResourceRequirements: &v1.ResourceRequirements{
			GPU:    &v1.GPUResourceRequirements{Count: &gpuCount},
			CPU:    &v1.CPUResourceRequirements{Quantity: "500m"},
			Memory: &v1.MemoryResourceRequirements{Quantity: "1Gi"},
		},
		Policy: &v1.SandboxPolicy{
			Version: 2,
			Filesystem: &v1.FilesystemPolicy{
	...
	assert.Equal(t, "500m", p.ResourceRequirements.Cpu.GetQuantity())
	assert.Equal(t, "1Gi", p.ResourceRequirements.Memory.GetQuantity())
