  ...
  it('rawSpec reaches an ungated field and overrides a curated one', async () => {
    let created: {
      spec?: {
        ...
        providers?: string[];
        resourceRequirements?: { cpu?: { quantity?: string } };
      };
    } = {};
    const sandbox = client({
      createSandbox: (req) => {
    ...
    await sandbox.create({
      image: 'curated-image',
      providers: ['claude'],
      resourceRequirements: { cpu: { quantity: '1' } },
      rawSpec: {
        logLevel: 'debug',
        template: { image: 'raw-image' },
        resourceRequirements: { cpu: { quantity: '4' } },
      },
    });
    ...
    expect(created.spec?.resourceRequirements?.cpu?.quantity).toBe('4');
  });
    ...
    const created = await templates.create(
      {
        ...
        spec: {
          workload: {
            ...
            resources: { cpu: { quantity: '1' }, memory: { quantity: '512Mi' }, gpu: { count: 1 } },
          },
